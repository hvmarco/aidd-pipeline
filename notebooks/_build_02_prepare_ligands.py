...
**If you do not want to authorize Google Drive**, change the line `USE_DRIVE = IS_COLAB` in the next cell to `USE_DRIVE = False` *before* running it. The notebook will still run end-to-end; caches go to `/content/aidd-pipeline/data/derived/` on the Colab session disk and **disappear when the runtime ends** — you'll have to redo the prep on every reconnect.
