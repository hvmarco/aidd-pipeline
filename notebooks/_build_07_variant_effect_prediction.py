...
**Step 15 introduces the swap mechanism.** `src/aidd/stability.py` now exposes `_build_rasp_full_cache(source)` which converts the upstream's 9 GB AlphaFold-based PRISM-format archive (`rasp_preds_alphafold_UP000005640_9606_HUMAN_v2_prism_dir.zip`) into a small `full_proteome_demo_set.parquet` covering all seven demo genes. The cache loader auto-detects this file when present; the public API of `rasp_ddg` / `rasp_covers` is unchanged. Operator step (one-off per machine): download the zip, call `_build_rasp_full_cache(zip_path, cache_dir=RASP_CACHE)`, write the ~1 MB resulting parquet to your Drive cache. After that, all six previously-uncovered demo genes resolve. The calibration block below was first run before this swap landed and is preserved for the methodology record; the DPYD I560S and KRAS G12C `RaSP=NaN` entries become real numbers once the new cache is in place.
"""),
