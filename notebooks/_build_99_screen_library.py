...
**Recommended GPU per target.** A100 is recommended for headline-demo targets with >500 residues — **DPYD** (~1025 aa) hits CUDA OOM on T4's 16 GB VRAM during Boltz-2 co-folding (verified on the step-17 closure run); **UGT1A1** (~533 aa) is borderline. T4 is sufficient for the smaller targets (**ERK2** ~360 aa, **KRAS** ~190 aa, **NAT2** ~290 aa, **CYP2D6** ~500 aa). The MoA helper (`_run_target_moa`, defined later in this notebook) degrades gracefully when Boltz-2 fails on a single (compound, variant) pair — the failed pair gets `error = "boltz_oom"` in `moa_reports/summary.csv` and a priors-only HTML report, while remaining demos continue. Full-coverage production runs across the headline-demo set should use A100; smaller-target runs are fine on T4.
