...
| 99  | `_build_99_screen_library.py`            | `99_screen_library.ipynb`           | end-to-end runner (`RUN_MODE` = `library` or `moa`; accepts `mutations=` list) | Colab Pro+ recommended | done (A100 closure verified on Colab Pro+ 2026-05-18; ERK2 library cache walk reproduces step 12 exactly — Shortlist 42 compounds @ TOP=0.10 rank_product_topk on rescorer_rf_proba_oof + boltz_affinity lanes; 5 MoA closure demos pass cleanly with 13 (compound × variant) HTML reports total — DPYD WT+5-FU + \*2A priors-only (out_of_scope_splice; 2 reports), UGT1A1 WT+irinotecan + \*28 priors-only via AlphaFold-DB fetch path (out_of_scope_promoter; 2 reports), CYP2D6 WT+\*10+tamoxifen + \*4 priors-only (3 reports; \*10 AlphaMissense=0.76/pathogenic, RaSP ΔΔG=+2.54 kcal/mol destabilising), KRAS WT+G12C+sotorasib (2 reports; Boltz-2's non-covalent affinity head predicts G12C ~2x tighter than WT: -0.892 vs -0.618 log10 IC50 µM — direction consistent with sotorasib's covalent design (pocket-geometry preference at the G12C site is captured) but Boltz-2 does NOT model the irreversible covalent bond formation, so the true WT-vs-G12C selectivity (effectively infinite in vitro / in cells) is substantially understated; signal = "pocket preference exists in the right direction", NOT "model captured covalent selectivity"), NAT2 WT+\*5+\*6+\*7+isoniazid deep walkthrough (4 reports; slow-acetylator stability-loss mechanism captured: RaSP ΔΔG +1.21 / +1.24 / +3.05 kcal/mol all destabilising; Boltz binding ~WT-like consistent with stability-driven not binding-driven phenotype); 0 of 13 (compound, variant) rows have populated `error` column in `moa_reports/summary.csv`; full coverage on A100, T4 OOMs on DPYD (~1025 aa) — the MoA helper degrades gracefully on T4 via the `boltz_failed_<class>` partial-HTML path added at commit 926368c; 2026-05-18) |
